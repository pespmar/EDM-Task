...
velocidad <- AllStations %>% 
  group_by(source)%>%
  summarise(vmedia = mean(vmedia))

arboles <- AllStations %>% 
  group_by(source)%>%
  summarise(n_arbol = mean(n_arbol))
      ...
      tabItem(tabName = "model",
              fluidRow(
                column(width = 6,
                       ...
                       p('This is a linear regression model that predicts the number of particles in a station for a concrete day, month an hour. It is not the value for the past year, it is an estimation taking into account all the available data from 2023'),
                       ...
                       p('In the graphics it is shown the average speed and the number of trees around          the station selected in comparison to the others. These factors could affect air         pollution ')
                ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$predict, {
    ...
    output$plot1 <- renderPlot({
      datos_filtrados <- velocidad[velocidad$source == input$source, ]
      
      p <- ggplot(velocidad, aes(x = source, y = vmedia)) +
        geom_bar(stat = "identity") +
        labs(title = "Average speed by station (km/h) ", x = "Station", y = "Avg. speed") +
        theme_minimal()
      
      if (!is.null(input$source)) {
        p <- p + geom_bar(data = datos_filtrados, stat = "identity", fill = "red")
      }
    ...
    output$plot2 <- renderPlot({
      datos_filtrados <- arboles[arboles$source == input$source, ]
      
      p <- ggplot(arboles, aes(x = source, y = n_arbol)) +
        geom_bar(stat = "identity") +
        labs(title = "Number of trees by station", x = "Station", y = "Number of trees") +
        theme_minimal()
      
      if (!is.null(input$source)) {
        p <- p + geom_bar(data = datos_filtrados, stat = "identity", fill = "green")
      }
